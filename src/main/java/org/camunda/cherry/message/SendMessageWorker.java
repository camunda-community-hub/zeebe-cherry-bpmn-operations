...
public class SendMessageWorker extends AbstractWorker {
    ...
    private static final String BPMNERROR_INCORRECT_VARIABLE="INCORRECT_VARIABLE";
    ...
    public SendMessageWorker() {
        super(WORKERTYPE_SEND_MESSAGE,
                Arrays.asList(
                        ...
                        RunnerParameter.getInstance(INPUT_MESSAGE_DURATION, "Duration (in ms)", Object.class, RunnerParameter.Level.OPTIONAL, "Message duration. After this delay, message is deleted if it doesn't fit a process instance")),
    ...
    private Map<String, Object> extractVariable(String variableList, final ActivatedJob activatedJob) {
        ...
        while (stVariable.hasMoreTokens()) {
            ...
            if (!stOneVariable.hasMoreTokens())
                throw new ZeebeBpmnError(BPMNERROR_INCORRECT_VARIABLE, "A variable must be <name>=<value>, no = for variable ["+name+"]");
            String value = (stOneVariable.hasMoreTokens() ? stOneVariable.nextToken() : null);
            if (value == null) {
                variables.put(name, getValue(value, null, activatedJob));
            } else
                variables.put(name, getValue(value, value, activatedJob));
        }
        return variables;
    }
